...
async function eoaClaimAndSwap(wallet: ethers.Wallet, provider: ethers.providers.JsonRpcProvider, sellToken: string, buyToken: string, claimAddress: string) {
    const { chainId } = await provider.getNetwork();
    console.log(`connected to chain ${chainId} with account ${wallet.address}`);
    const appData = generateAppData([buildClaimHook(provider, wallet.address, claimAddress), buildPermitHook(wallet, provider, SETTLEMENT_CONTRACT_ADDRESS, ethers.constants.MaxUint256,sellToken, chainId )], []);
    const orderConfig = {
        sellToken,
        buyToken,
...
const SETTLEMENT_CONTRACT_ADDRESS = "0x9008D19f58AAbD9eD0D60971565AA8510560ab41"
...
eoaClaimAndSwap(wallet, provider, CLAIM_TOKEN_ADDRESS, BUY_TOKEN_ADDRESS, CLAIM_CONTRACT_ADDRESS)